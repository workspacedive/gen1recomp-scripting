...
test('vendored love.js candidate exactly matches the pinned inventory', async () => {
  ...
  assert.equal(LOVEJS_RUNTIME.id, 'lovejs-11.5-r16')
  assert.equal(LOVEJS_RUNTIME.adapterVersion, 16)
...
test('host normalization overlay preserves upstream bytes and supplies reviewed compatibility hooks', async () => {
  ...
  assert.match(adapter, /"setLooping", "setFilter", "setPitch"/)
  assert.match(adapter, /__hostNativeSource = nativeSource/)
