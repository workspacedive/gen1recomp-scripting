...
test('vendored love.js candidate exactly matches the pinned inventory', async () => {
  ...
  assert.equal(LOVEJS_RUNTIME.id, 'lovejs-11.5-r17')
  assert.equal(LOVEJS_RUNTIME.adapterVersion, 17)
...
test('host normalization overlay preserves upstream bytes and supplies reviewed compatibility hooks', async () => {
  ...
  assert.match(adapter2, /facade\.setLooping = function/)
  assert.match(adapter2, /facade\.setFilter = function/)
  assert.match(adapter2, /facade\.setPitch = function/)
  assert.match(adapter2, /__hostNativeSource = nativeSource/)
