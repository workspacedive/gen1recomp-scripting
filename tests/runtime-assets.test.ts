...
test('vendored love.js candidate exactly matches the pinned inventory', async () => {
  ...
  assert.equal(LOVEJS_RUNTIME.id, 'lovejs-11.5-r11')
  assert.equal(LOVEJS_RUNTIME.adapterVersion, 11)
...
test('host normalization overlay preserves upstream bytes and supplies reviewed compatibility hooks', async () => {
  ...
  assert.match(adapter, /nativeNewQueueableSource = audio and audio\.newQueueableSource/)
  assert.match(adapter, /newQueueableSource returned \[/)
