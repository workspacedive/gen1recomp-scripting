...
> **Ehrlicher Status:** Die Scripting-App 0.10.0 besitzt vier native Tabs und getrennte persistente Library-/Mod-/Payload-Speicher. love.js/LÖVE 11.5 und der Gen1Recomp-0.3.20-Payload bis `Module.postrun` sind auf dem echten Gerät **VERIFIZIERT**. Runtime r5 ergänzt außerhalb beider Upstreams eine getestete reine-Lua-Bit-Kompatibilität und einen expliziten experimentellen Start für genau eine erneut verifizierte Library-ROM über `POKEPORT_IMPORT_ROM`. Launcher, ROM-Import, Gameplay, Eingabe, Audio, Saves und Lifecycle mit r5 sind bis zum nächsten Gerätetest **NICHT VERIFIZIERT**.
...
- [`docs/architecture/conformance-audit-0.3.0.md`](docs/architecture/conformance-audit-0.3.0.md) bis [`0.10.0`](docs/architecture/conformance-audit-0.10.0.md) — explizite Prüfungen jeder Iteration gegen die vereinbarte Architektur.
