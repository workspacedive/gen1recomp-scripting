...
> **Ehrlicher Status:** Die Scripting-App 0.10.1 besitzt vier native Tabs und getrennte persistente Library-/Mod-/Payload-Speicher. love.js/LÖVE 11.5 und der Gen1Recomp-0.3.20-Payload bis `Module.postrun` sind auf dem echten Gerät **VERIFIZIERT**. Der Bit-Blocker ist beim experimentellen ROM-Start beseitigt und der offizielle Import erreicht `bootGame` (**TEILWEISE VERIFIZIERT**). Ein von upstream verdeckter Fehler in `Game.load` führt danach zu `Game.stack == nil`; Runtime r6 bewahrt für den nächsten Gerätetest dessen ursprünglichen Traceback. Gameplay bleibt **NICHT VERIFIZIERT**.
...
- [`docs/architecture/conformance-audit-0.3.0.md`](docs/architecture/conformance-audit-0.3.0.md) bis [`0.10.1`](docs/architecture/conformance-audit-0.10.1.md) — explizite Prüfungen jeder Iteration gegen die vereinbarte Architektur.
