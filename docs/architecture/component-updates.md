...
- since 0.4.0, an explicit user action may stage the built-in approved Gen1Recomp 0.3.20 pin. Fixed URL, version, filename, byte length and SHA-256 must all match; the complete `.love` ZIP is preflighted; `Version.lua` literals are gated; the staged copy is hashed again; and publication is a directory rename. Its state is `stored-runtime-gated`: no active pointer, mount, evaluation or restart is created.
